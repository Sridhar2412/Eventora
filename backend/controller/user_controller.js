...
const updateUser = async (req, res) => {
  ...
  const { mobile, fullname, email, password, dob, role, organization } =
    req.body; // Get the new user data from the request body
  ...
  try {
    ...
    if (updated) {
      return res
        .status(200)
        .json({ status: "OK", code: "200", data: "User updated successfully" });
    } else {
      return res.status(400).json({
        status: "ERROR",
        code: "400",
...
async function login(req, res) {
  ...
  if (!body.email || !body.password)
    return res.status(400).json({
      status: "ERROR",
      code: "400",
      error: { message: "Enter username and password" },
    });
  ...
  const password = await User.findOne({
    where: { password: body.password.toString() },
  });
  if (!password)
    return res.status(400).json({
      status: "ERROR",
  ...
  const user = await User.findOne({
    where: { email: body.email, password: body.password.toString() },
  });
  // console.log(user);
  if (user) var token = setUser(user);
  return res
    .status(200)
    .json({ status: "OK", code: "200", data: { accessToken: token } });
}
